...
$$ P = \begin{pmatrix} 
    p_{00} & 1-p_{00} \\
    1-p_{11} & p_{11}
\end{pmatrix}, \hspace{0.2cm} p_{00}, p_{11} \sim \text{Beta}(32, 10) $$
...
The choices of $\theta_1$ and $\theta_2$ are partly informed by the preliminary empirical observation in figure \ref{fig:data_descr} where 
the histogram of log-returns indicates that low-volatility returns are highly concentrated near zero, while extreme returns in the tails 
are better captured by a higher-variance normal distribution. 
...
Convergence of the Markov chain Monte Carlo sampler was evaluated using standard diagnostics, including rank-normalized $\hat{R}$ statistics and visual inspection 
...
\begin{minipage}{0.48\textwidth}
    ...
    \hline\hline
     & Mean & 2.5\% & 97.5\% \\
    \hline
    Low-volatility ($\sigma_1$)  & 0.0061 & 0.0058 & 0.0065 \\
    High-volatility ($\sigma_2$) & 0.0188 & 0.0176 & 0.0201 \\
    \hline\hline
    \end{tabular}
...
yielding a narrow credible interval with posterior mean $\bar{\sigma}_1 = 0.0061$  $[0.0058, 0.0065]$. In contrast, the posterior for the high-volatility regime, $\sigma_2$, exhibits substantially greater dispersion, with posterior 
mean $\bar{\sigma}_2 = 0.0188$ $[0.0176, 0.0201]$ and a noticeably wider credible interval.
...
\begin{minipage}{0.48\textwidth}
    ...
    \hline\hline
     & Mean & 2.5\% & 97.5\% \\
    \hline
    Low-volatility ($p_{00}$)  & 0.9701 & 0.9583 & 0.9803 \\
    High-volatility ($p_{11}$) & 0.9331 & 0.9058 & 0.9549 \\
    \hline\hline
    \end{tabular}
...
exhibits a posterior mean of 0.97, implying highly persistent stable states. In contrast, the probability of remaining in the high-volatility regime, $p_{11}$,
has a lower posterior mean of 0.9331, indicating that turbulent market conditions are less persistent and tend to revert more quickly. Consistent with posterior 
...
$$ T(Y) = P(|Y_t| > q_{0.9}(|Y|)) $$
where $q_{0.9}(|Y|)$ denotes the empirical 90th percentile of the absolute observed returns. The resulting posterior predictive p-value of 0.8087 indicates that replicated datasets 
tend to exhibit slightly more extreme behavior than observed, providing quantitative support for the visual assessment while remaining within acceptable bounds for model adequacy.
...
\par
In contrast, posterior inference for the transition probabilities exhibits modest sensitivity to prior choice. Under the diffuse $\text{Beta}(1,1)$ prior, posterior means 
for both $p_{00}$ and $p_{11}$ are slightly higher than those obtained under the more informative $\text{Beta}(32,10)$ specification. This pattern suggests that the 
informative prior exerts meaningful shrinkage toward lower persistence, leading to conservative estimates of regime duration. The data, however, favor even 
higher state persistence than originally encoded, particularly for the high-volatility regime.
...
\par
From a methodological perspective, achieving reliable MCMC convergence in the presence of latent state processes requires careful prior specification and substantial computational effort. 
...
Trace plots and autocorrelation diagnostics indicate satisfactory mixing across all chains for parameters of inferential interest; convergence summaries are reported in Table~\ref{tab:mcmc_diagnostics}.
...
\begin{table}[h]
    ...
    \begin{tabular}{l c c c}
        ...
        \midrule 
        $\tau$ & [Value] & [Value] & [Value] \\
        $\theta_1$ & [Value] & [Value] & [Value] \\
        $\theta_2$ & [Value] & [Value] & [Value] \\
        $\sigma_1$ & [Value] & [Value] & [Value] \\
        $\sigma_2$ & [Value] & [Value] & [Value] \\
        $p_{00}$ & [Value] & [Value] & [Value] \\
        $p_{11}$ & [Value] & [Value] & [Value] \\
        $Z$ & [Value] & [Value] & [Value] \\
